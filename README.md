...
I had an idea in mind that I start developing only to realise that it was not fully doable with Spotify's API.

Since I invested some time in this code, and it was not well documented on Spotify (not for python at least) I decided to share this code maybe someone will make use of it.

Below is the code of a Flask (Python3) based example on how to get a user's Token to access their data. In addition to requesting the refresh token when needed.
...
Developed by [TheInkedEngineer](http://www.theinkedengineer.com)
